      ...
      {fields.map(({ key, label, type, placeholder }) => (
        <div key={key}>
          ...
          <input type={type} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })}
            className={`w-full px-4 py-2.5 rounded-xl border text-sm outline-none transition-all dark:bg-gray-700 dark:text-white ${errors[key] ? 'border-red-400 bg-red-50 dark:bg-red-900/30' : 'border-gray-300 dark:border-gray-600 focus:border-green-500 focus:ring-2 focus:ring-green-100 dark:focus:ring-green-900/30'}`}
